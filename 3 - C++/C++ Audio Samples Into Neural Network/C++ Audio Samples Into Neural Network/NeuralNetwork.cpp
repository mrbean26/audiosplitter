...
void NeuralNetwork::train(int epochs, float learningRate, float momentum,
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        for (int t = 0; t < trainSize; t++) {
            ...
            float currentTotalError = accumulate(errors.begin(), errors.end(), 0.0f);
            totalcurrent += currentTotalError;

            cout << t << " " << trainSize << " " << accumulate(errors.begin(), errors.end(), 0.0f) << endl;

            // add derivatives & change weights
            setDerivativesAndDeltaWeights(errors, learningRate, momentum);
        }

        epochTotalErrors.push_back(totalcurrent / trainSize);
        
    }
    cout << "Error across all epochs: " << vectorToString(epochTotalErrors) << endl;
    system("pause");
}
...
void NeuralNetwork::saveWeights(string directory) {
    int networkSize = network.size();
    ofstream outputFile;

    outputFile.open(directory + "outWeights.txt");
    if (!outputFile) {
        cout << "File cannot be opened: " << directory.c_str() << endl;
        return;
    }

    for (int layerNum = 1; layerNum < networkSize; layerNum += 2) {
        int currentLayerSize = network[layerNum].size();
        string currentOutputLine = "";

        for (int weight = 0; weight < currentLayerSize; weight++) {
            cout << "Saving layer: " << layerNum << " Weight: " << weight << " of " << currentLayerSize << endl;

            outputFile << to_string(network[layerNum][weight]);
            if (weight < currentLayerSize - 1) {
                outputFile << ",";
            }
        }
...
vector<float> splitCommaSeparatedFloats(string used) {
    vector<float> result;
    stringstream inputStream(used);

    while (inputStream.good()) {
        string subString;
        getline(inputStream, subString, ',');

        result.push_back(stof(subString));
    }
    return result;
}

void NeuralNetwork::loadWeights(string directory){
    int networkSize = network.size();
    ifstream inputFile;

    inputFile.open(directory + "outWeights.txt");
    if (!inputFile) {
        cout << "File cannot be opened: " << directory.c_str() << endl;
        return;
    }
    ...
    while (getline(inputFile, currentLine)) {
        vector<float> currentLineWeights = splitCommaSeparatedFloats(currentLine);

        int currentNetworkLayerSize = network[lineIndex + 1].size();
        for (int weight = 0; weight < currentNetworkLayerSize; weight++) {
            network[lineIndex + 1][weight] = currentLineWeights[weight];
        }

        cout << "Loaded layer number: " << lineIndex << endl;
